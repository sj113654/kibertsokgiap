...
def make_table(soup):
    ...
    table = [stk, name, revenue, percentage_this_month, percentage_accumulate]
    return table
...
for stk in cos:
    ...
    kim_iong_koo_form_data = {"firstin": "true", "step": "0", "off": "1", "TYPEK": "sii", "isnew": "true", "id": "t05st10_ifrs_form", "co_id": stk}
    soup = get_soup(api_url, form_data)
    kim_iong_koo_soup = get_soup(api_url, kim_iong_koo_form_data)
    try:
        table.append(make_table(soup))
    except:
        table.append(make_table(kim_iong_koo_soup))
